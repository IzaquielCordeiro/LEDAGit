...
public class SortComparatorBSTImpl<T extends Comparable<T>> extends BSTImpl<T> implements SortComparatorBST<T>
{
   ...
   protected void insert(BSTNode<T> node, T element)
   {
      ...
      {
         if (this.comparator.compare(element, node.getData()) > 0) insert((BSTNode<T>) node.getRight(), element);

         else if (this.comparator.compare(element, node.getData()) < 0) insert((BSTNode<T>) node.getLeft(), element);
      }
   }
   ...
   protected BSTNode<T> search(BSTNode<T> node, T element)
   {
      ...
      else if (this.comparator.compare(element, node.getData()) == 0) nodeResult = node;

      else if (this.comparator.compare(element, node.getData()) > 0) nodeResult = search((BSTNode<T>) node.getRight(), element);
   ...
   {
      while (!this.isEmpty()) this.remove(this.root.getData());
   ...
   {
      @SuppressWarnings("unchecked") T[] arrayResult = (T[]) new Comparable[this.size()];
