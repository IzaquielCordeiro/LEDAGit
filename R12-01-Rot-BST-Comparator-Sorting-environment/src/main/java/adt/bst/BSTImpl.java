...
public class BSTImpl<T extends Comparable<T>> implements BST<T>
{
	...
	protected BSTNode<T> sucessor(BSTNode<T> node)
	{
		...
		{
			if (!sucessor.isEmpty() && sucessor.getRight().equals(node)) sucessor = sucessor((BSTNode<T>) node.getParent());
		}
	...
	protected BSTNode<T> predecessor(BSTNode<T> node)
	{
		...
		if (node.getParent() != null)
		{
			if (!predecessor.isEmpty() && predecessor.getLeft().equals(node))predecessor = predecessor((BSTNode<T>) node.getParent());
		}
